    ...
    <div>
      ...
      {items.length === 0 ? (
        <div className="card" style={{ textAlign: 'center', padding: '3rem' }}>
          ...
          <p style={{ color: 'var(--text-muted)', marginBottom: '1rem' }}>ยังไม่มีประวัติ</p>
          ...
          {items.map((item: any) => (
            <Link key={item.id} to={`/result/${item.id}`} style={{ textDecoration: 'none', color: 'inherit' }}>
              <div className="list-item">
                <div>
                  <span className={`badge badge-${item.riskLevel}`}>{item.riskLevel}</span>
                  <span style={{ marginLeft: '0.75rem', fontWeight: 600 }}>Score: {item.riskScore}</span>
                </div>
